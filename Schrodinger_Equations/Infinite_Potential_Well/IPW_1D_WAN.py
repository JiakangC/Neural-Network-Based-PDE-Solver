...
def train_seperate(n, L=2.0, epochs=3000, lr=0.001, layers=[1, 50, 50, 50, 1], LBFGS=False, method='WAN', technique='FBC'):
    # Define the network
    ...
    weight_pde = 1.0
    weight_norm = 1.0
    weight_bc = 1000.0  # Force the boundary condition
    if technique == 'OG':
        weight_orth = 10000.0
    else:
        weight_orth = 0.0
    u_layers = layers
    v_layers = [1, 20, 20, 20, 1]
    enforce_bc = True if technique in ['OG', 'FBC'] else False
    u_model = FCN(u_layers, num_states=n, L=L, enforce_bc=enforce_bc).to(device)
    ...
    if best_state_u is not None:
        path_u = os.path.join(save_path, f'{method}_best_model_u_n{n}_FBC_Orthogonal_{timestamp}.pth')
        path_v = os.path.join(save_path, f'{method}_best_model_v_n{n}_FBC_Orthogonal_{timestamp}.pth')
    ...
    results = {
        ...
        'L2_error': L2_error,
        'min_epoch': min_epoch,
        'best_model_path': os.path.join(save_path, f'{method}_best_model_u_n{n}_BC_Orthogonal_{timestamp}.pth'),
        'losses': os.path.join(save_path, f'{method}_losses_n{n}_BC_Orthogonal_{timestamp}.npy'),
        'L2_errors': os.path.join(save_path, f'{method}_L2_errors_n{n}_BC_Orthogonal_{timestamp}.npy'),
    ...
    best_model = FCN(u_layers, num_states=n, L=L, enforce_bc=enforce_bc).to(device)
    ...
    plt.legend()
    plt.grid()
    plt.savefig(os.path.join(save_path, f'WAN_solution_n{n}_{technique}.png'))
    plt.close()
    # plotting loss
    ...
    plt.legend()
    plt.grid()
    plt.savefig(os.path.join(save_path, f'WAN_loss_n{n}_{technique}.png'))
    plt.close()
    # plot the L2 error
    plt.figure(figsize=(10, 5))
    ...
    plt.title('L2 Error over Epochs')
    plt.legend()
    plt.grid()
    plt.savefig(os.path.join(save_path, f'WAN_L2_error_n{n}_{technique}.png'))
    plt.close()
    return u_model
...
def run_seperate_method(n_values, epochs=3000, LBFGS=False):
    for n in n_values:
        ...
        model_wan_bc = train_seperate(n, epochs=epochs, LBFGS=LBFGS, method='WAN', technique='BC')
        model_wan_fbc = train_seperate(n, epochs=epochs, LBFGS=LBFGS, method='WAN', technique='FBC')
        model_wan_og = train_seperate(n, epochs=epochs, LBFGS=LBFGS, method='WAN', technique='OG')
        print(f"Training completed for n={n}.")

if __name__ == "__main__":
    n_values = [1, 2, 3, 4, 5]  # You can adjust this list for more eigenstates
    run_seperate_method(n_values, epochs=20, LBFGS=False)