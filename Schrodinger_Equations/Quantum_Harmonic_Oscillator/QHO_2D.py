...
save_path = 'results/Quantum_Harmonic_Oscillator_2D'
if not os.path.exists(save_path):
    os.makedirs(save_path)

results_file = os.path.join(save_path, 'results_QHO_2D.json')
...
def train_pinn_seperate(nx, ny, L=6.0, epochs=10000, lr=0.001, LBFGS=False, method='PINN', technique='FBC'):
    # Define the network
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if method == 'WAN':
            ...
            if method == 'PINN':
                ...
                residual = -0.5 * (d2u_dx2 + d2u_dy2) + V * u_interior - E * u_interior
...
if __name__ == "__main__":
    run_seperate_method(10000, LBFGS=False)